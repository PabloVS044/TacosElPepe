...
SELECT * FROM v_resumen_pedidos WHERE estado_pedido IN ('aprobado', 'en_proceso');
SELECT * FROM v_resumen_pedidos WHERE estado_pago = 'pendiente';
SELECT * FROM v_resumen_pedidos ORDER BY fecha_creacion DESC LIMIT 20;
SELECT * FROM v_resumen_pedidos WHERE id_cliente = 2 ORDER BY fecha_creacion DESC;
...
SELECT * FROM v_stock_critico ORDER BY deficit DESC;
SELECT COUNT(*) AS alertas_activas FROM v_stock_critico;
